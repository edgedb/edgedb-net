...
internal sealed class HttpDuplexer : IBinaryDuplexer
{
    ...
    public HttpDuplexer(EdgeDBHttpClient client)
    {
        ...
        _packetReadTCS = new TaskCompletionSource();
    ...
    public ValueTask DisconnectAsync(CancellationToken token = default) => ValueTask.CompletedTask;

    public async IAsyncEnumerable<DuplexResult> DuplexAsync([EnumeratorCancellation] CancellationToken token = default,
    ...
    private Task<HttpResponseMessage> SendInternalAsync(CancellationToken token = default, params Sendable[] packets)
    {
        ...
        message.Headers.Authorization = new AuthenticationHeaderValue("Bearer", _client.AuthorizationToken);
        message.Content.Headers.ContentType = new MediaTypeHeaderValue(ContentTypeHeader);
