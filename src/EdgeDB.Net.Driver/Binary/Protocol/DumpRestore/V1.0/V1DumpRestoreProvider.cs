...
internal class V1DumpRestoreProvider : IDumpRestoreProvider
{
    ...
    public static readonly byte[] DUMP_FILE_FORMAT_BLOB =
    {
        0xff, 0xd8, 0x00, 0x00, 0xd8,

        // file format "EDGEDB"
        69, 68, 71, 69, 68, 66, 0x00,
    ...
    public unsafe (Protocol.V1._0.Packets.Restore Restore, IEnumerable<RestoreBlock> Blocks) ReadDatabaseDump(
        Stream stream)
    {
        if (!stream.CanRead)
        ...
        if (version > DumpRestoreVersion.Major)
        ...
        restore = new Restore {HeaderData = dumpHeader.Raw};
        ...
        while (stream.Position < stream.Length)
        {
            ...
            blocks.Add(new RestoreBlock {BlockData = dumpBlock.Raw});
        }

        return (restore!, blocks);
    }

    private static unsafe IReceiveable ReadPacket(Stream stream)
    {
