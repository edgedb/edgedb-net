...
public sealed class EdgeDBClient : IEdgeDBQueryable, IAsyncDisposable
{
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyCollection<TResult?>> QueryAsync<TResult>(string query,
    ...
    /// <inheritdoc />
    public async Task<TResult> QueryRequiredSingleAsync<TResult>(string query,
        IDictionary<string, object?>? args = null,
        Capabilities? capabilities = Capabilities.Modifications, CancellationToken token = default)
    {
        await using var client = await GetOrCreateClientAsync(token).ConfigureAwait(false);
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyCollection<Json>> QueryJsonElementsAsync(string query,
        IDictionary<string, object?>? args = null,
        Capabilities? capabilities = Capabilities.Modifications, CancellationToken token = default)
    {
        await using var client = await GetOrCreateClientAsync(token).ConfigureAwait(false);
    ...
    private async Task<BaseEdgeDBClient> CreateClientAsync(ulong id, CancellationToken token = default)
    {
        switch (_poolConfig.ClientType)
        {
            case EdgeDBClientType.Tcp:
            {
                ...
                async ValueTask OnConnect(BaseEdgeDBClient _)
                {
                    ...
                    if (
                        ...
                        ServerConfig.Count != client.ServerConfig.Count ||
                        ServerConfig.Except(client.ServerConfig).Any())
                    {
            ...
            case EdgeDBClientType.Http:
            {
                ...
                if (
                    ServerConfig is null ||
                    ServerConfig.Count != client.ServerConfig.Count || ServerConfig.Except(client.ServerConfig).Any())
                {
                    ServerConfig = client.ServerConfig;
                }
