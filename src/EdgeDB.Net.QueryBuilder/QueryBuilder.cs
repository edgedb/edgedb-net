...
public partial class QueryBuilder<TType, TContext> : IQueryBuilder<TType, TContext> where TContext : IQueryContext
{
    ...
    /// <inheritdoc />
    public ValueTask<CompiledQuery> CompileAsync(IEdgeDBQueryable edgedb, bool debug = false,
        ...
        Capabilities? capabilities, CancellationToken token)
    {
        ...
        return await edgedb.QueryAsync<TType>(result.Query, result.RawVariables, capabilities, token)
    ...
    {
        var result = await IntrospectAndCompileAsync(edgedb, false, token).ConfigureAwait(false);
        return await edgedb.QuerySingleAsync<TType>(result.Query, result.RawVariables, capabilities, token)
            .ConfigureAwait(false);
    }

    /// <inheritdoc />
    async Task<TType?> IMultiCardinalityExecutable<TType>.ExecuteSingleAsync(IEdgeDBQueryable edgedb,
        Capabilities? capabilities, CancellationToken token)
    {
        var result = await IntrospectAndCompileAsync(edgedb, false, token).ConfigureAwait(false);
        return await edgedb.QuerySingleAsync<TType>(result.Query, result.RawVariables, capabilities, token)
    ...
    /// <inheritdoc />
    async Task<TType> IMultiCardinalityExecutable<TType>.ExecuteRequiredSingleAsync(IEdgeDBQueryable edgedb,
        Capabilities? capabilities, CancellationToken token)
    {
        var result = await IntrospectAndCompileAsync(edgedb, false, token).ConfigureAwait(false);
        return await edgedb.QueryRequiredSingleAsync<TType>(result.Query, result.RawVariables, capabilities, token)
            .ConfigureAwait(false);
    }
    ...
    private QueryBuilder<TTarget, TContext> EnterNewType<TTarget>()
    {
        ...
        return new QueryBuilder<TTarget, TContext>(_state);
    }
        ...
        where TNewContext : IQueryContext
    {
        ...
        return new QueryBuilder<TType, TNewContext>(_state);
    }
        ...
        where TNode : QueryNode
    {
        ...
        var builder = new NodeBuilder(context, QueryGlobals, Nodes, QueryVariables) {IsAutoGenerated = autoGenerated};
    ...
    internal void CompileInternal(QueryWriter writer, CompileContext? context = null)
    {
        ...
        if (context.IncludeGlobalsInQuery && QueryGlobals.Any())
        {
            ...
            if (with is null)
            {
                ...
                with = new WithNode(builder) {SchemaInfo = SchemaInfo};
            ...
            writer.Term(
                ...
                with.GetType().Name,
                null,
                new QueryNodeMetadata(with),
                Token.Of(writer => with.FinalizeQuery(writer))
            );
    ...
    /// </returns>
    private async ValueTask<CompiledQuery> IntrospectAndCompileAsync(IEdgeDBQueryable edgedb, bool debug,
        CancellationToken token)
    {
        if (Nodes.Any(x => x.RequiresIntrospection) ||
            QueryGlobals.Any(x => x.Value is SubQuery subQuery && subQuery.RequiresIntrospection))
            SchemaInfo ??= await SchemaIntrospector.GetOrCreateSchemaIntrospectionAsync(edgedb, token)
                .ConfigureAwait(false);
    ...
    private QueryBuilder<TType, TContext> Else(Func<IQueryBuilder<TType, TContext>, IMultiCardinalityQuery<TType>> func)
    {
        ...
        {
            Nodes = new List<QueryNode>(), Variables = new Dictionary<string, object?>()
        });
    ...
    ///     The current node does not support else statements.
    /// </exception>
    private QueryBuilder<TType, TContext> Else(
        Func<IQueryBuilder<TType, TContext>, ISingleCardinalityQuery<TType>> func)
    {
        if (CurrentUserNode is not InsertNode insertNode)
            throw new InvalidOperationException($"Cannot else on a {CurrentUserNode}");
