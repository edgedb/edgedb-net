...
internal static class InitializationTranslator
{
    ...
    private static SubQuery? GenerateMultiLinkInserter(Type innerType, IEnumerable collection,
        ExpressionContext context)
    {
        var methodDef = typeof(InitializationTranslator)
            .GetMethods(BindingFlags.Static | BindingFlags.NonPublic).FirstOrDefault(x =>
                x.ContainsGenericParameters && x.Name == nameof(GenerateMultiLinkInserter));
        ...
        QueryWriter writer)
    {
        for (var i = 0; i != expressions.Count; i++)
        {
            ...
            switch (expression)
            {
                ...
                {
                    if (disassembled.Last() is ConstantExpression constant &&
                        disassembled[^2] is MemberExpression constParent)
                    {
                        if (!property.Type.IsAssignableTo(typeof(IEnumerable)))
                            throw new NotSupportedException(
                                $"cannot use {property.Type} as a multi link collection type; its not assignable to IEnumerable.");
