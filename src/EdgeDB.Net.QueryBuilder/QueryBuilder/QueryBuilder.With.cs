...
{
    public new static IQueryBuilder<TType, QueryContextSelfVars<TType, TVariables>> With<TVariables>(
        TVariables variables)
        => new QueryBuilder<TType, QueryContextSelfVars<TType, TVariables>>().With(variables);

    public static IQueryBuilder<TType, QueryContextSelfVars<TType, TVariables>> With<TVariables>(
        Expression<Func<QueryContextSelf<TType>, TVariables>> variables)
...
{
    IQueryBuilder<TType, QueryContextSelfVars<TType, TVariables>> IQueryBuilder<TType, TContext>.
        With<TVariables>(TVariables variables) => With(variables);

    IQueryBuilder<TType, QueryContextSelfVars<TType, TVariables>> IQueryBuilder<TType, TContext>.With<TVariables>(
        Expression<Func<QueryContextSelf<TType>, TVariables>> variables) => WithInternal<TVariables>(variables);
    ...
    public QueryBuilder<TType, QueryContextSelfVars<TType, TVariables>> With<TVariables>(TVariables variables)
    {
        ...
        foreach (var property in typeof(TVariables).GetProperties())
        {
            ...
            else if (ReflectionUtils.IsSubclassOfRawGeneric(typeof(JsonReferenceVariable<>), property.PropertyType))
            {
                ...
                QueryVariables.Add(jsonVarName, Json.Serialize(referenceValue));
