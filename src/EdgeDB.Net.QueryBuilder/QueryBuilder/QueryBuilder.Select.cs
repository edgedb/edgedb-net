...
public static partial class QueryBuilder
{
    ...
    /// </returns>
    public static ISelectQuery<TType, QueryContextSelf<TType>> SelectExpression<TType>(
    ...
    /// </returns>
    public static ISelectQuery<TType, QueryContextSelf<TType>> SelectExpression<TType>(
        Expression<Func<QueryContext, TType?>> expression)
...
public partial class QueryBuilder<TType, TContext>
{
    ...
    public ISelectQuery<TResult, TContext> Select<TResult>(Action<ShapeBuilder<TResult>> shape)
    {
        ...
        AddNode<SelectNode>(new SelectContext(typeof(TResult))
        {
            Shape = shapeBuilder, IsFreeObject = typeof(TResult).IsAnonymousType()
        });
        return EnterNewType<TResult>();
    }
    ...
    public ISelectQuery<TNewType, TContext> SelectExpression<TNewType>(Expression<Func<TNewType?>> expression)
    {
        AddNode<SelectNode>(new SelectContext(typeof(TType))
        {
            Expression = expression, IncludeShape = false, IsFreeObject = typeof(TNewType).IsAnonymousType()
        });
    ...
    ISelectQuery<TType, TContext> ISelectQuery<TType, TContext>.OrderBy<U>(Expression<Func<TType, U>> propertySelector,
        OrderByNullPlacement? nullPlacement)
        => OrderBy(true, propertySelector, nullPlacement);

    ISelectQuery<TType, TContext> ISelectQuery<TType, TContext>.OrderByDescending<U>(
    ...
    ISelectQuery<TType, TContext> ISelectQuery<TType, TContext>.OrderBy<U>(
        ...
        where TNewContext : IQueryContext
    {
        ...
        {
            shape(shapeBuilder = new ShapeBuilder<TNew>());
        }

        AddNode<SelectNode>(new SelectContext(typeof(TNew))
        {
            Shape = shapeBuilder, IsFreeObject = typeof(TNew).IsAnonymousType()
        });
    ...
    )
    {
        ...
        AddNode<SelectNode>(new SelectContext(typeof(TType))
        {
            Expression = expression, Shape = shapeBuilder, IsFreeObject = typeof(TNewType).IsAnonymousType()
        });
    ...
    )
    {
        ...
        AddNode<SelectNode>(new SelectContext(typeof(TType))
        {
            Expression = expression, Shape = shapeBuilder, IsFreeObject = typeof(TShape).IsAnonymousType()
        });
    ...
    internal ISelectQuery<TNewType, TContext> SelectExp<TNewType, TInitContext>(
        Expression<Func<TInitContext, TNewType?>> expression)
    {
        AddNode<SelectNode>(new SelectContext(typeof(TType))
        {
            Expression = expression, IncludeShape = false, IsFreeObject = typeof(TNewType).IsAnonymousType()
