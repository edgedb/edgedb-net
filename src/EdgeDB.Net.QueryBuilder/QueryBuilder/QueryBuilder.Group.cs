...
public partial class QueryBuilder<TType, TContext>
{
    ...
    IGroupQuery<TResult, TContext> IQueryBuilder<TType, TContext>.Group<TResult>(
        Expression<Func<TContext, TResult>> selector)
        => GroupInternal<TResult, TContext>(selector);

    IGroupQuery<TResult, TContext> IQueryBuilder<TType, TContext>.Group<TResult>(Expression<Func<TResult>> selector,
        Action<ShapeBuilder<TResult>> shape)
        => GroupInternal<TResult, TContext>(selector, shape);

    IGroupQuery<TResult, TContext> IQueryBuilder<TType, TContext>.Group<TResult>(
        Expression<Func<TContext, TResult>> selector, Action<ShapeBuilder<TResult>> shape)
        => GroupInternal<TResult, TContext>(selector, shape);

    IMultiCardinalityExecutable<Group<TKey, TType>> IGroupQuery<TType, TContext>.By<TKey>(
        Expression<Func<TType, TKey>> selector)
        => By(selector).EnterNewType<Group<TKey, TType>>();

    IMultiCardinalityExecutable<Group<TKey, TType>> IGroupQuery<TType, TContext>.By<TKey>(
        Expression<Func<TType, TContext, TKey>> selector)
        => By(selector).EnterNewType<Group<TKey, TType>>();
    ...
    {
        ShapeBuilder<TResult>? shapeBuilder = shape is not null ? new ShapeBuilder<TResult>() : null;
        shape?.Invoke(shapeBuilder!);

        AddNode<GroupNode>(new GroupContext(typeof(TType)) {Selector = selector, Shape = shapeBuilder});
