...
{
    IUpdateQuerySet<TType, TContext> IUpdateQuery<TType, TContext>.Filter(
        Expression<Func<TType, TContext, bool>> filter)
        => Filter(filter);
    ...
    IMultiCardinalityExecutable<TType> IUpdateQuerySet<TType, TContext>.Set<TAnon>(
        Expression<Func<TType, TContext, TAnon>> shape)
        => Set(UpdateShapeBuilder<TType, TContext>.FromInitExpression(shape));
    ...
    {
        var updateNode = AddNode<UpdateNode>(new UpdateContext(typeof(TSelected)) {Selector = selector});
