...
internal partial class InsertNode : QueryNode<InsertContext>
{
    ...
    /// </exception>
    private ShapeDefinition BuildInsertShape(Type? shapeType = null,
        Union<LambdaExpression, InsertValue, IJsonVariable>? shapeValue = null)
    {
        ...
        foreach (var (name, value) in insertValue.Values)
        {
            ...
            if (property.CustomConverter is not null)
            {
                ...
                setters.Add(new ShapeSetter(writer => writer
                    ...
                    .Append(" := ")
                    .QueryArgument(new Token(scalar), varName)
                ));
                continue;
            }
                ...
                (value?.Equals(ReflectionUtils.GetDefault(property.Type)) ?? false))
            {
                setters.Add(new ShapeSetter((writer, info) =>
                {
                    ...
                    writer
                        .Append(property.EdgeDBName)
                        .Append(" := ")
                        .QueryArgument(new Token(edgeqlType), varName, optional: !edgedbProp.Required);
                }));
                continue;
            }
            ...
            if (isScalar)
            {
                ...
                setters.Add(new ShapeSetter(writer => writer
                    .Append(property.EdgeDBName)
                    .Append(" := ")
                    .QueryArgument(new Token(edgeqlType), varName, optional: value is null)
                ));
                continue;
            }
                ...
                else if (isArray) // if its a multi link
                {
                    ...
                    setters.Add(new ShapeSetter(writer => writer
                        ...
                        .Shape(
                            ...
                            debug: Defer.This(
                                () => $"Shape of property {property.PropertyName} of type {property.Type}")
                        )));
    ...
    public void UnlessConflictOn(LambdaExpression selector) => _unlessConflictExpression ??=
        writer => writer.Append(" unless conflict on ", ProxyExpression(selector));
    ...
    public void Else(IQueryBuilder builder)
    {
        ...
        _elseStatement = writer => writer
            .Append(" else ")
            .Wrapped(writer =>
                builder.WriteTo(writer, this,
                    new CompileContext {IncludeAutogeneratedNodes = false, IncludeGlobalsInQuery = false})
            );
    }
    ...
    internal readonly struct InsertValue(
        Type type,
        IDictionary<string, object?> values,
        EdgeDBPropertyMapInfo? propertyMapInfo)
    {
