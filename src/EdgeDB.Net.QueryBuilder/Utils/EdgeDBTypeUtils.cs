...
internal static class EdgeDBTypeUtils
{
    ...
    public static bool TryGetScalarType(Type type, [MaybeNullWhen(false)] out EdgeDBTypeInfo info)
    {
        ...
        var enumerableType = ReflectionUtils.IsSubclassOfRawGeneric(typeof(IEnumerable<>), type)
            ? type
            : type.GetInterfaces()
                .FirstOrDefault(x => ReflectionUtils.IsSubclassOfRawGeneric(typeof(IEnumerable<>), x));
        ...
        if (scalar != null)
            info = new EdgeDBTypeInfo(type, scalar, false, child);
        else if (hasChild)
            info = new EdgeDBTypeInfo(type, "array", true, child);
        else if (type.IsEnum)
            info = new EdgeDBTypeInfo(type, type.GetEdgeDBTypeName(), false, child);
    ...
    /// </param>
    /// <param name="innerLinkType">
    ///     The inner type of the multi link if <paramref name="isMultiLink" /> is
    ...
    public static bool IsLink(Type type, out bool isMultiLink, [NotNullWhen(true)] out Type? innerLinkType)
    {
        ...
        var enumerableType = ReflectionUtils.IsSubclassOfRawGeneric(typeof(IEnumerable<>), type) ? type : null;
        if (type != typeof(string) && (enumerableType is not null || (enumerableType = type.GetInterfaces()
                .FirstOrDefault(x => ReflectionUtils.IsSubclassOfRawGeneric(typeof(IEnumerable<>), x))) != null))
        {
    ...
    ///     <see langword="true" /> if the given type is a link; otherwise <see langword="false" />.
    /// </returns>
    public static bool IsLink(EdgeDBPropertyInfo info, out bool isMultiLink,
        [MaybeNullWhen(false)] out Type? innerLinkType)
    {
