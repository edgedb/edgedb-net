...
internal static class QueryGenerationUtils
{
    ...
    /// </returns>
    public static async ValueTask<IEnumerable<PropertyInfo>> GetPropertiesAsync<TType>(IEdgeDBQueryable edgedb,
        bool? exclusive = null, bool? @readonly = null, CancellationToken token = default)
    {
        var introspection = await SchemaIntrospector.GetOrCreateSchemaIntrospectionAsync(edgedb, token)
    ...
    /// </exception>
    public static IEnumerable<PropertyInfo> GetProperties(SchemaInfo schemaInfo, Type type, bool? exclusive = null,
        bool? @readonly = null, bool includeId = false)
    {
    ...
    /// </returns>
    public static async ValueTask<Expression<Func<TType, TType>>> GenerateUpdateFactoryAsync<TType>(
        IEdgeDBQueryable edgedb, TType value, CancellationToken token = default)
    {
    ...
    /// </returns>
    public static async ValueTask<Expression<Func<TType, QueryContextSelf<TType>, bool>>>
        GenerateUpdateFilterAsync<TType>(IEdgeDBQueryable edgedb, TType value, CancellationToken token = default)
    {
        ...
        var exclusiveProperties = await GetPropertiesAsync<TType>(edgedb, true, token: token).ConfigureAwait(false);

        var unsafeLocalMethod = typeof(QueryContextSelf<TType>).GetMethod("UnsafeLocal", 0, new[] {typeof(string)})!;
