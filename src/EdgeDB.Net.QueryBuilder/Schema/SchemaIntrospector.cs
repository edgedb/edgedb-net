...
internal class SchemaIntrospector
{
    ...
    /// </returns>
    public static ValueTask<SchemaInfo> GetOrCreateSchemaIntrospectionAsync(IEdgeDBQueryable edgedb,
    ...
    private static async Task<SchemaInfo> IntrospectSchemaAsync(IEdgeDBQueryable edgedb, CancellationToken token)
    {
        ...
        var result = await QueryBuilder
                    ...
                    shape.Computeds((ctx, prop) => new
                    {
                        Cardinality = (string)ctx.UnsafeLocal<object>("cardinality") == "One"
                            ? ctx.UnsafeLocal<bool>("required")
                                ? DataTypes.Cardinality.One
                                : DataTypes.Cardinality.AtMostOne
                            : ctx.UnsafeLocal<bool>("required")
                                ? DataTypes.Cardinality.AtLeastOne
                                : DataTypes.Cardinality.Many,
            ...
            .ExecuteAsync(edgedb, token: token);
