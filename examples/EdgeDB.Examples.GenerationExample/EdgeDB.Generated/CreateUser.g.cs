...
// Generated on 2022-12-06T20:36:05.1149047Z
#nullable enable
using EdgeDB;
...
ELSE (SELECT Person)";
    ...
    public static Task<UserModel> ExecuteAsync(IEdgeDBQueryable client, string? name, string? email, CancellationToken token = default)
        => client.QueryRequiredSingleAsync<UserModel>(Query, new Dictionary<string, object?>() { { "name", name }, { "email", email } }, capabilities: (Capabilities)1ul, token: token);
    ...
    /// <param name="token">A cancellation token used to cancel the asyncronous query.</param>
    /// <returns>A Task representing the asynchronous query operation. The result of the task is the result of the query.</returns>
    public static Task<UserModel> CreateUserAsync(this IEdgeDBQueryable client, string? name, string? email, CancellationToken token = default)
