.. _gel-dotnet-datatypes:

Datatypes
=========

The .NET driver maps the following Gel types to the corresponding:

.. note:: 

  Currently, Gel does not support unsigned numbers. Using them with the
...
+------------------------------------+------------------------------------+
| :eql:type:`json`                   | ``DataTypes.Json``                 |
+------------------------------------+------------------------------------+
| :eql:type:`uuid`                   | ``Guid``                           |
+------------------------------------+------------------------------------+
| :eql:func:`range`                  | ``DataTypes.Range<T>`` \*\*        |
+------------------------------------+------------------------------------+
...
of Gel types is microseconds, while .NETs are nanoseconds. This means that 
when converting from Gel to .NET, the precision will be lost. When converting 
from .NET to Gel, the precision will be rounded to the nearest microsecond.
...
other Gel range type that isn't ``int32`` will not work. This is only 
available in >=1.0.5